...
def register_records_tools(mcp: FastMCP) -> None:
    ...
    async def list_patient_records(
        ...
        updated_after: Annotated[
            int | None, "Only return records updated after this epoch (seconds)"
        ] = None,
        offset: Annotated[
            str | None, "Pagination token (next_token from a previous response)"
        ] = None,
        ...
        """
        await ctx.info(
            f"[list_patient_records] Listing records for patient: {patient_id}"
        )
        ...
        try:
            ...
            result = await records_service.list_patient_records(
                patient_id, updated_after, offset
            )
    ...
    async def get_patient_record(
        ...
        document_id: Annotated[
            str, "Record/document unique identifier (from list_patient_records)"
        ],
        ctx: Context = CurrentContext(),
    ) -> dict[str, Any]:
        """
        ...
        """
        await ctx.info(
            f"[get_patient_record] Getting record {document_id} for patient: {patient_id}"
        )
    ...
    async def delete_patient_record(
        ...
        document_id: Annotated[
            str,
            "Record/document unique identifier to delete (from list_patient_records)",
        ],
        ctx: Context = CurrentContext(),
    ) -> dict[str, Any]:
        """
        ...
        """
        await ctx.info(
            f"[delete_patient_record] Deleting record {document_id} for patient: {patient_id}"
        )
        ...
        try:
            ...
            records_service = RecordsService(client)
            result = await records_service.delete_patient_record(
    ...
    async def upload_patient_record(
        patient_id: Annotated[str, "Patient ID (oid from list/mobile lookup)"],
        file_path: Annotated[
            str | None, "Absolute path to a local file to upload (PDF, image, etc.)"
        ] = None,
        file_url: Annotated[
            str | None,
            "Public https URL of the file to upload (PDF or JPEG/PNG image, max 5MB)",
        ] = None,
        title: Annotated[str | None, "Human-readable title (max 256 chars)"] = None,
        tags: Annotated[
            list[str] | None, "Tags for the record (max 10, each 2-20 chars)"
        ] = None,
        document_type: Annotated[
            str | None, "Document type code, e.g. 'lr' for lab report"
        ] = None,
        document_date: Annotated[
            int | None, "Document reference date as epoch seconds"
        ] = None,
        ctx: Context = CurrentContext(),
    ) -> dict[str, Any]:
        """
        ...
        to a patient's medical record vault. Provide either file_url (the URL of
        a file the user shared, e.g. an attachment link; PDF or JPEG/PNG image,
        max 5MB) or a local file_path. At least one is required; if both are
        given, file_url is used. Pass the URL exactly as provided - never
        ...
        Args:
            patient_id: Patient's unique identifier (oid)
            file_path: Optional absolute path to the local file to upload
            file_url: Optional public https URL of a PDF or image (JPEG/PNG), max 5MB
        ...
        try:
            ...
            await ctx.info(
                f"Uploaded patient record successfully: {result.get('document_id')}"
            )
