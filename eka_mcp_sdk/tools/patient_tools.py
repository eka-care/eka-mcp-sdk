...
def register_patient_tools(mcp: FastMCP) -> None:
    ...
    async def search_patients(
        prefix: Annotated[
            str,
            "Search prefix to match against patient profiles (username, mobile, or full name)",
        ],
        limit: Annotated[
            int | None, "Maximum number of results to return (default: 50, max: 50)"
        ] = None,
        select: Annotated[
            str | None,
            "Comma-separated list of additional fields to include (dob, gen) as needed by the client",
        ] = None,
        ...
        if not get_supports_elicitation():
            prefix = get_whatsapp_mobile_number()
        ...
        try:
            ...
            patient_count = (
                len(result.get("patients", [])) if isinstance(result, dict) else 0
            )
            await ctx.info(
                f"[search_patients] Found {patient_count} patients matching search criteria\n"
            )
    ...
    async def get_patient_details_basic(
        patient_id: Annotated[str | None, "Patient's unique identifier"] = None,
        ctx: Context = CurrentContext(),
    ) -> dict[str, Any]:
        """
        ...
        """
        await ctx.info(
            f"[get_patient_details_basic] Getting basic patient details for: {patient_id}"
        )
        ...
        try:
            ...
            await ctx.info("[get_patient_details_basic] Completed successfully\n")
    ...
    async def get_comprehensive_patient_profile(
        ...
        include_appointments: Annotated[
            bool, "Include appointments (default: True)"
        ] = True,
        appointment_limit: Annotated[int | None, "Limit appointments"] = None,
        ctx: Context = CurrentContext(),
    ) -> dict[str, Any]:
        """
        ...
        """
        await ctx.info(
            f"[get_comprehensive_patient_profile] Getting comprehensive profile for patient: {patient_id}"
        )
        await ctx.debug(
            f"Include appointments: {include_appointments}, limit: {appointment_limit}"
        )
        ...
        try:
            ...
            await ctx.info(
                "[get_comprehensive_patient_profile] Completed successfully\n"
            )

            return {"success": True, "data": result}
        except EkaAPIError as e:
            await ctx.error(
                f"[get_comprehensive_patient_profile] Failed: {e.message}\n"
            )
            return {
                "success": False,
                "error": {
    ...
    async def add_patient(
        patient_data: PatientData, ctx: Context = CurrentContext()
    ) -> dict[str, Any]:
        """
        ...
        if not get_supports_elicitation():
            mobile_number = get_whatsapp_mobile_number()
            patient_data.mobile = mobile_number
        ...
        await ctx.info("[add_patient] Creating new patient profile")
        ...
        try:
            ...
            await ctx.info(
                f"[add_patient] Completed successfully - patient ID: {patient_id}\n"
            )
    ...
    async def list_patients(
        ...
        page_size: Annotated[
            int | None, "Records per page (default: 500, max: 2000)"
        ] = None,
        select: Annotated[
            str | None, "Additional fields (default: dob,gen for full details)"
        ] = "dob,gen",
        from_timestamp: Annotated[int | None, "Filter: created after timestamp"] = None,
        ...
        """
        await ctx.info(
            f"[list_patients] Listing patients - page {page_no}, size: {page_size or 'default'}"
        )
        ...
        try:
            ...
            await ctx.info(
                f"[list_patients] Completed successfully - retrieved {patient_count} patients\n"
            )
    ...
    async def update_patient(
        update_data: Annotated[
            dict[str, Any],
            "Dictionary of fields and values to update (e.g., name, mobile, dob)",
        ],
        patient_id: Annotated[
            str | None, "Unique identifier of the patient to update"
        ] = None,
        ctx: Context = CurrentContext(),
    ) -> dict[str, Any]:
        """
        ...
        """
        await ctx.info(
            f"[update_patient] Updating patient {patient_id} - fields: {list(update_data.keys())}"
        )
        ...
        try:
            ...
            await ctx.info("[update_patient] Completed successfully\n")
    ...
    async def get_patient_by_mobile(
        ...
        full_profile: Annotated[
            bool, "Return full profile if True (default: False)"
        ] = False,
        ctx: Context = CurrentContext(),
    ) -> dict[str, Any]:
        """
        ...
        if not get_supports_elicitation():
            mobile = get_whatsapp_mobile_number()
    ...
    async def mobile_number_verification(
        mobile_number: Annotated[
            str, "Mobile number to verify (10 digits without country code)"
        ],
        otp: Annotated[
            str | None, "One-Time Password sent to the mobile number"
        ] = None,
        stage: Annotated[
            Literal["send_otp", "verify_otp"], "Stage of verification"
        ] = "send_otp",
        ctx: Context = CurrentContext(),
    ) -> dict[str, Any]:
        """
        ...
        await ctx.info(
            f"[mobile_number_verification] {stage_display} for: {mobile_number}"
        )
        ...
        try:
            ...
            patient_service = PatientService(client)
            result = await patient_service.mobile_number_verification(
                mobile_number, otp, stage
            )
    ...
    async def authentication_elicitation(
        method: Annotated[
            Literal["mobile", "email"], "Authentication method to use"
        ] = "mobile",
        mobile_number: Annotated[
            str | None, "Mobile number to verify (10 digits without country code)"
        ] = None,
        email_address: Annotated[str | None, "Email address of the user."] = None,
        ctx: Context = CurrentContext(),
    ) -> dict[str, Any]:
        """
        ...
        await ctx.info(
            f"[authentication_elicitation] Initiating {method} authentication for mobile: {mobile_number}, email: {email_address}"
        )
        ...
        try:
            ...
            patient_service = PatientService(client)
            return await patient_service.authentication_elicitation(
    ...
    async def list_all_patient_profiles(
        ...
        try:
            ...
            await ctx.info("[list_all_patient_profiles] Retrieved patient profiles\n")
            return {"success": True, "data": result}
        except EkaAPIError as e:
    ...
    async def get_patient_vitals(
        patient_id: Annotated[str | None, "Patient's unique identifier (oid)"] = None,
        ctx: Context = CurrentContext(),
    ) -> dict[str, Any]:
        """
        ...
        """
        await ctx.info(
            f"[get_patient_vitals] Fetching vitals for patient: {patient_id}"
        )
        ...
        try:
            ...
            await ctx.info("[get_patient_vitals] Retrieved vitals successfully\n")
            return {"success": True, "data": result}
        except EkaAPIError as e:
    ...
    try:
        ...
        if isinstance(appointments_data, list):
            ...
            if "appointments" in appointments_data:
                ...
                appointments_list = (
                    [appointments_data]
                    if appointments_data.get("appointment_id")
                    else []
                )
        ...
        logger.warning(f"Failed to enrich patient appointments: {e!s}")
        # Return original data if enrichment fails
        if isinstance(appointments_data, list):
            return appointments_data
        elif (
            isinstance(appointments_data, dict) and "appointments" in appointments_data
        ):
